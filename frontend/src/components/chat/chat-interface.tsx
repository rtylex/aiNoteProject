...
export function ChatInterface({ documentId, sessionId, isFullWidth = false }: { documentId: string, sessionId?: string, isFullWidth?: boolean }) {
    ...
    const handleSend = async () => {
        ...
        const userMsg: Message = { id: Date.now(), sender: 'user', message: input }
                            ...
                            <div className="flex flex-col items-center justify-center h-[50vh] text-center text-muted-foreground opacity-50">
                                {(documentStatus === 'pending' || documentStatus === 'processing') ? (
                                    <>
                                        ...
                                        <p className="text-lg font-medium">Doküman hazırlanıyor...</p>
                                        <p className="text-sm">OCR işlemi tamamlandığında sohbete başlayabilirsiniz.</p>
                                    </>
                                ) : (
                                    <>
                                        <p className="text-lg font-medium">Bu dokümanla sohbet etmeye başla</p>
                                        <p className="text-sm">Sorularını sor, özet iste veya analiz yap.</p>
                                    </>
                                )}
                            </div>
                        )}
